...
public abstract class GShape implements Cloneable, Serializable {
	...
	public void keepRotating(Graphics2D graphics2d, int x, int y) {
		double rrAnchorX = this.shape.getBounds().getCenterX();
		double rrAnchory = this.shape.getBounds().getCenterY();
		
		double radian = Math.atan2(rrAnchory-y, rrAnchorX-x)-Math.atan2(rrAnchory-this.py, rrAnchorX-this.px);
		//double a =  Math.sqrt(Math.pow(this.px-rrAnchorX, 2)+Math.pow(this.py-rrAnchory,2))/Math.sqrt(Math.pow(x-rrAnchorX,2)+Math.pow(y-rrAnchory,2));
		//(x-this.shape.getBounds().getX()+this.shape.getBounds().getWidth()/2)/(this.shape.getBounds().getY()+this.shape.getBounds().getHeight()/2-y);
		
		this.affineTransform.setToRotation(radian, this.shape.getBounds().getCenterX(), this.shape.getBounds().getCenterY());//.rotate(radian-oldRadian, this.shape.getBounds().getCenterX(), this.shape.getBounds().getCenterY());
//		this.affineTransform.rotate(Math.atan(a), this.shape.getBounds().getCenterX(), this.shape.getBounds().getCenterY());
		this.shape = this.affineTransform.createTransformedShape(this.shape);
		//this.anchors = this.affineTransform.createTransformedShape(this.anchors);
		this.oldRadian=radian;
		this.px = x;
		this.py = y;
	}
	...
	public void keepShearing(Graphics2D graphics2d, int x, int y) {
		...
		if(x>=this.shape.getBounds().getX() && x <= (this.shape.getBounds().getX() + this.shape.getBounds().getWidth())) {
		...
		this.shape = this.affineTransform.createTransformedShape(this.shape);
		this.anchors.changeGG(graphics2d, this.affineTransform, x);
		this.px = x;
		this.py = y;
		}
	}
