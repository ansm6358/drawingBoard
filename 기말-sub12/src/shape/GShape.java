...
//		graphics2d.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
	...
	public void keepResizing(Graphics2D graphics2d, int x, int y) {
		...
		case NW:
			...
			if (rateW > 0 && rateH > 0) {
				...
				changeShape(bx,by,-1,-1);
				this.seteAnchor(eAnchor.SE);
				changeAnchor = true;
			} else if (rateW <= 0) {
				changeShape(bx,0,-1,1);
				this.seteAnchor(eAnchor.NE);
				changeAnchor = true;
			} else if (rateH <= 0) {
				changeShape(0,by,1,-1);
	...
	private void changeShape(int x, int y, int rx, int ry) {
	...
	public void keepShearing(Graphics2D graphics2d, int x, int y) {
		...
		double rateShear = (ox - bx) / (this.shape.getBounds().getWidth() / 2);

		if (ox < this.shape.getBounds().getX()) {
			ox = (int) this.shape.getBounds().getX();
		} else if (this.px > (this.shape.getBounds().getX() + this.shape.getBounds().getWidth())) {
			ox = (int) (this.shape.getBounds().getX() + this.shape.getBounds().getWidth());
		}

		if (bx >= this.shape.getBounds().getX()
				&& bx <= (this.shape.getBounds().getX() + this.shape.getBounds().getWidth())) {
			...
			this.shape = this.affineTransform.createTransformedShape(this.shape);
			this.anchors.changeGG(graphics2d, this.affineTransform, this.shape.getBounds().getCenterX(),
