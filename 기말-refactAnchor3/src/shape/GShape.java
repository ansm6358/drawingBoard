...
//			}
	...
	public void keepResizing(Graphics2D graphics2d, int x, int y) {
		...
		double NWX = this.anchors.getNWX();
		double NWY = this.anchors.getNWY();
		...
		case NN:
			...
			break;
		case NE:
			rateW = (x - NWX) / this.shape.getBounds().getWidth();
			rateH = (this.py - y + this.shape.getBounds().getHeight()) / this.shape.getBounds().getHeight();
			settingX = NWX - (rateW * NWX);
			settingY = this.py - (rateH * this.shape.getBounds().getY());
			if (rateW > 0 && rateH > 0) {
				doResize(rateW, rateH, settingX, settingY);
			} else if (rateW <= 0 && rateH <= 0) {
...
//			this.affineTransform.setToRotation(-this.rotateRate, this.shape.getBounds().getCenterX(), this.shape.getBounds().getCenterY());
//			this.shape = this.affineTransform.createTransformedShape(this.shape);
//		}
		this.affineTransform.setToRotation(-this.rotateRate, this.shape.getBounds().getCenterX(), this.shape.getBounds().getCenterY());
		this.shape = this.affineTransform.createTransformedShape(this.shape);
...
//		}
	...
	public void keepShearing(Graphics2D graphics2d, int x, int y) {
		...
		this.shape = this.affineTransform.createTransformedShape(this.shape);
		this.anchors.changeGG(graphics2d, this.affineTransform, this.shape.getBounds().getBounds().getCenterX(), this.shape.getBounds().getBounds().getCenterY());
		this.px = x;
		this.py = y;
		}
	}
