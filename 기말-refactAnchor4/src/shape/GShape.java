...
public abstract class GShape implements Cloneable, Serializable {
	...
	public void keepResizing(Graphics2D graphics2d, int x, int y) {
		...
		this.affineTransform.setToRotation(-this.rotateRate, this.shape.getBounds().getCenterX(),
				this.shape.getBounds().getCenterY());
		this.shape = this.affineTransform.createTransformedShape(this.shape);
		double cx = (int) this.shape.getBounds().getCenterX();
		double cy = (int) this.shape.getBounds().getCenterY();
		...
		case NW:
			...
			settingX = ox - (rateW * this.shape.getBounds().getX());
			settingY = oy - (rateH * this.shape.getBounds().getY());
			if (rateW > 0 && rateH > 0) {
